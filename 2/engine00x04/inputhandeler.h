...
struct Cam3 Input_update3(struct Cam3 cam);
struct Cam4 Input_update4(struct Cam4 cam);
...
struct Cam3 Input_update3(struct Cam3 cam){
	//if(cam.grounded==1){
		...
		if(input[4]==1){
			cam.vel.x+=cam.up.x*speed;
			cam.vel.y+=cam.up.y*speed;
			cam.vel.z+=cam.up.z*speed;
	//	}
	}
	if(input[5]==1){
		cam.vel.x-=cam.up.x*speed;
...
struct Cam4 Input_update4(struct Cam4 cam){
	//if(cam.grounded==1){
		struct Vec4 vec=Vec4_ortho(cam.ana,cam.rght,cam.up);
		if(input[0]==1){
			cam.vel.w+=vec.w*speed;
			cam.vel.x+=vec.x*speed;
			cam.vel.y+=vec.y*speed;
			cam.vel.z+=vec.z*speed;
		}
		if(input[1]==1){
			cam.vel.w-=cam.rght.w*speed;
			cam.vel.x-=cam.rght.x*speed;
			cam.vel.y-=cam.rght.y*speed;
			cam.vel.z-=cam.rght.z*speed;
		}
	...
	if(!gui){
		if(mouse[1]<0){
			if(Vec4_angle(cam.fwd,cam.up)<M_PI/8){}else{
				cam.fwd=Vec4_rot(cam.fwd,cam.fwd,cam.up,mouse[1]/-50);}
		}else{
			if(Vec4_angle(cam.fwd,cam.up)>7*M_PI/8){}else{
				cam.fwd=Vec4_rot(cam.fwd,cam.fwd,cam.up,mouse[1]/-50);}
		}
		cam.fwd=Vec4_rot(cam.fwd,cam.fwd,cam.rght,mouse[0]/-50);
		mouse[0]=0;
		mouse[1]=0;
	}
	if(input[6]==1){
		if(Vec4_angle(cam.fwd,cam.up)<M_PI/8){}else{
			cam.fwd=Vec4_rot(cam.fwd,cam.fwd,cam.up,0.05);}
	}
	if(input[7]==1){
		if(Vec4_angle(cam.fwd,cam.up)>7*M_PI/8){}else{
			cam.fwd=Vec4_rot(cam.fwd,cam.fwd,cam.up,-0.05);}
	...
	if(input[8]==1){
		cam.fwd=Vec4_rot(cam.fwd,cam.fwd,cam.rght,-0.05);
	}
	if(input[9]==1){
		cam.fwd=Vec4_rot(cam.fwd,cam.fwd,cam.rght,0.05);
	}
...
cam.rght=Vec4_ortho(cam.up,cam.fwd,cam.ana);
